...
class ProductRepository {
  ...
  Future<Page<Product>> list({
    ...
    if (data is Map) {
      ...
      items = asList(
        m['items'] ?? m['data'] ?? m['productos'] ?? m['rows'] ?? m['result'] ?? m['results'] ?? [],
      );
      total =
          (m['total'] ?? m['count'] ?? m['totalCount'] ?? m['total_count'] ?? items.length) as int? ??
  ...
  Future<Product> getById(int id) async {
    ...
    final m =
        (data is Map && data['data'] != null) ? asMap(data['data']) : asMap(data);
  ...
  Future<Product> create(
    ...
    try {
      ...
      if (kDebugMode) {
        ...
        print(
          '   imagen: ${imageBytes != null ? "${imageBytes.length} bytes" : imageFile != null ? "File" : "null"}',
        );
      }
