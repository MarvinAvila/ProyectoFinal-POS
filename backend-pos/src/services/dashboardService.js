...
const dashboardService = {
    ...
    async getResumenCompleto() {
        ...
        const [
            ...
            productosRentablesRaw,
            ventasPorEmpleadoRaw,
            distribucionInventarioRaw,
            alertasRecientesRaw,
            ventasPorDiaRaw,
            crecimientoMensualRaw
        ] = await Promise.all([
            dashboardRepository.getVentasHoyStats(),
            dashboardRepository.getTotalProductos(),
            dashboardRepository.getTotalUsuariosActivos(),
            dashboardRepository.getVentasMesStats(),
            dashboardRepository.getTotalAlertasPendientes(),
            dashboardRepository.getTotalCategorias(),
            dashboardRepository.getTotalProveedores(),
            dashboardRepository.getVentasRecientes(),
            dashboardRepository.getProductosPopulares('7 days'),
            dashboardRepository.getProductosRentables('30 days'),
            dashboardRepository.getVentasPorEmpleado('30 days'),
            dashboardRepository.getDistribucionInventario(),
            dashboardRepository.getAlertasRecientes(),
            dashboardRepository.getVentasUltimaSemana(),
            dashboardRepository.getCrecimientoMensual()
        ]);
        ...
        const ventasRecientes = ventasRecientesRaw.map(row => Venta.fromDatabaseRow(row).toJSON());
        ...
        const alertasRecientes = alertasRecientesRaw.map(row => Alerta.fromDatabaseRow(row).toJSON());
    ...
    async getEstadisticasAvanzadas(queryParams) {
        ...
        const estadisticasAvanzadas = {
            ...
            metricas_eficiencia: {
                ...
                frecuencia_compra: parseFloat(metricasEficiencia.frecuencia_compra) || 0,
            },
        ...
        estadisticasAvanzadas.resumen = {
            ...
            ventas_totales: estadisticasAvanzadas.ventas_por_dia.reduce((sum, v) => sum + v.total_ventas, 0),
            ingresos_totales: estadisticasAvanzadas.ventas_por_dia.reduce((sum, v) => sum + v.ingresos, 0),
        };
    ...
    async getDatosVentas(queryParams) {
        const { tipo_grafico = "lineas", periodo = "7d", agrupar_por = "dia" } = queryParams;
        const interval = _getInterval(periodo);
        ...
        const [
            ventasResult,
            metodosPagoResult,
            ventasPorCategoriaResult
        ] = await Promise.all([
            dashboardRepository.getVentasPorPeriodoAgrupado(interval, groupByClause),
            dashboardRepository.getMetodosPago(interval),
            dashboardRepository.getVentasPorCategoria(interval)
        ]);

        const resumen = {
            total_ventas: ventasResult.reduce((sum, v) => sum + parseInt(v.total_ventas), 0),
            ingresos_totales: ventasResult.reduce((sum, v) => sum + parseFloat(v.ingresos), 0),
            ticket_promedio: ventasResult.reduce((sum, v) => sum + parseFloat(v.promedio_venta), 0) / (ventasResult.length || 1),
        };
        ...
        const metodos_pago = metodosPagoResult.map(row => ({
            ...
            total_ventas: parseInt(row.total_ventas) || 0,
            ingresos: parseFloat(row.ingresos) || 0,
            porcentaje: resumen.ingresos_totales > 0 ? (parseFloat(row.ingresos) / resumen.ingresos_totales) * 100 : 0,
        }));
    ...
    async getEstadoInventario() {
        ...
        const estadisticas = {
            ...
            productos_por_caducar: parseInt(inventarioStats.productos_por_caducar) || 0,
        };
        ...
        return {
            ...
            alertas: {
                necesita_atencion: estadisticas.productos_agotados + estadisticas.productos_stock_bajo + estadisticas.productos_por_caducar,
    ...
    async getMetricasFinancieras() {
        ...
        const margenes_ganancia = margenGananciaRaw.map(row => ({
            ...
            ingresos: parseFloat(row.ingresos) || 0,
            costos: parseFloat(row.costos) || 0,
            margen_bruto: (parseFloat(row.ingresos) || 0) - (parseFloat(row.costos) || 0),
        ...
        return {
            ...
            resumen: {
                ...
                margen_promedio: margenes_ganancia.reduce((sum, row) => sum + row.margen_porcentaje, 0) / (margenes_ganancia.length || 1),
    ...
    async getEstadisticasUsuarios() {
        ...
        const resumen = {
            total_usuarios: usuariosPorRol.reduce((sum, row) => sum + parseInt(row.total), 0),
            usuarios_activos: usuariosPorRol.reduce((sum, row) => sum + parseInt(row.activos), 0),
            ventas_ultimo_mes: actividadReciente.reduce((sum, row) => sum + parseInt(row.total_ventas), 0),
            ingresos_ultimo_mes: actividadReciente.reduce((sum, row) => sum + parseFloat(row.ingresos_generados), 0),
        };
    ...
    async getReporteAlertas() {
        ...
        return {
            ...
            alertas_pendientes: alertas.reduce((sum, row) => sum + parseInt(row.pendientes), 0),
            timestamp: new Date().toISOString(),
        };
    }
};
...
function _getGroupByClause(agrupar_por) {
    switch (agrupar_por) {
        case "semana": return `DATE_TRUNC('week', fecha)`;
        case "mes": return `DATE_TRUNC('month', fecha)`;
        case "año": return `DATE_TRUNC('year', fecha)`;
        default: return `DATE(fecha)`;
    }
}
...
function _obtenerGastosParaMes(gastos, mes) {
    const gastoMes = gastos.find(g => new Date(g.mes).getTime() === new Date(mes).getTime());
...
function _calcularProyeccionAnual(ingresosMensuales) {
    ...
    const ingresoPromedio = ingresosMensuales.reduce((sum, row) => sum + row.ingresos, 0) / ingresosMensuales.length;
