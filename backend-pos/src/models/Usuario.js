...
class Usuario {
   ...
   */
  async validatePassword(contrasenaPlana) {
  ...
  toJSON() {
    return {
      ...
      ultimo_login: this.ultimo_login, // ← Nuevo
  ...
  getPerfilPublico() {
    return {
      ...
      ultima_sesion: this.ultimo_login,
      actualizado_en: this.actualizado_en,
    };
  }
  ...
  static fromDatabaseRow(row) {
    return new Usuario(
      ...
      row.activo,
      row.creado_en,
      row.ultimo_login, // ← Nuevo
      row.token_recuperacion, // ← Nuevo
      row.expiracion_token, // ← Nuevo
      row.actualizado_en
    );
  }
  ...
  static validate(usuarioData, esActualizacion = false) {
    ...
    if (
      ...
      typeof usuarioData.activo !== "boolean"
    ) {
  ...
  /**
   * Valida solo la contraseña (sin otros campos del usuario)
   */
  static validarSoloContrasena(contrasena) {
    const errors = [];
