...
const authController = {
    ...
    async changePassword(req, res) {
        ...
        try {
            ...
            if (!contrasena_actual || !nueva_contrasena) {
                ...
                return responseHelper.error(res, 'Contraseña actual y nueva contraseña son obligatorias', 400);
            }
            ...
            const contrasenaActualValida = await usuario.verificarContrasena(contrasena_actual);
            ...
            if (validationErrors.length > 0) {
                await client.query('ROLLBACK');
                return responseHelper.error(res, `Error en la nueva contraseña: ${validationErrors.join(', ')}`, 400);
            }
    ...
    async register(req, res) {
        ...
        try {
            ...
            if (validationErrors.length > 0) {
                await client.query('ROLLBACK');
                return responseHelper.error(res, `Errores de validación: ${validationErrors.join(', ')}`, 400);
            }
            ...
            if (passwordErrors.length > 0) {
                await client.query('ROLLBACK');
                return responseHelper.error(res, `Error en la contraseña: ${passwordErrors.join(', ')}`, 400);
            }
            ...
            const result = await client.query(
                ...
                [usuario.nombre, usuario.correo, contrasenaHash, usuario.rol, usuario.activo, usuario.creado_en]
            );
            ...
            logger.audit('Usuario registrado', req.user?.id_usuario || 'sistema', 'CREATE_USER', {
    ...
    async forgotPassword(req, res) {
        ...
        try {
            ...
            if (!correo) {
                return responseHelper.error(res, 'El correo electrónico es requerido', 400);
            }
            ...
            if (result.rows.length === 0) {
                ...
                return responseHelper.success(res, null, 'Si el correo existe, se enviarán instrucciones');
            }
            ...
            logger.audit('Solicitud de recuperación de contraseña', usuario.id_usuario, 'FORGOT_PASSWORD', {
                ip: req.ip,
                tokenGenerado: true
            });
            ...
            return responseHelper.error(res, 'Error procesando solicitud de recuperación', 500, error);
        } finally {
            client.release();
        }
    },
    ...
    async resetPassword(req, res) {
        ...
        try {
            ...
            if (!token || !nueva_contrasena) {
                await client.query('ROLLBACK');
                return responseHelper.error(res, 'Token y nueva contraseña son requeridos', 400);
            }
            ...
            logger.audit('Contraseña restablecida via token', usuario.id_usuario, 'RESET_PASSWORD', {
                ip: req.ip,
                metodo: 'token_recuperacion'
            });

            return responseHelper.success(res, null, 'Contraseña restablecida correctamente');
            ...
            return responseHelper.error(res, 'Error restableciendo contraseña', 500, error);
        } finally {
            client.release();
        }
    },
    ...
    async obtenerEstadisticasUsuario(usuarioId) {
        try {
            ...
            return {
                ...
                total_ventas_monto: parseFloat(ventasResult.rows[0].total_ventas_monto) || 0,
                alertas_pendientes: parseInt(alertasResult.rows[0].alertas_pendientes) || 0
            };
        } catch (error) {
            logger.error('Error obteniendo estadísticas de usuario', { usuarioId, error: error.message });
            return {};
        }
    }
};
