...
const detalleVentaController = {
    ...
    try {
      ...
      if (error.message === "ID inválido") {
        return responseHelper.error(
          res,
          "ID inválido en parámetros de búsqueda",
          400
        );
      }
  ...
  async create(req, res) {
    ...
    try {
      ...
      if (
        ...
        return responseHelper.error(
          res,
          "Faltan datos obligatorios: id_venta, id_producto, cantidad, precio_unitario",
          400
        );
      }
      ...
      if (validationErrors.length > 0) {
        ...
        return responseHelper.error(
          res,
          "Errores de validación en el detalle",
          400,
          {
      ...
      if (error.message === "ID inválido") {
        return responseHelper.error(
          res,
          "ID inválido en los datos proporcionados",
          400
        );
      }
  ...
  async update(req, res) {
    ...
    try {
      ...
      const precioUnitario =
        newPrecio != null ? parseFloat(newPrecio) : oldDetalle.precio_unitario;
      const cantidadFinal =
        newCantidad != null ? parseFloat(newCantidad) : oldCantidad;
      const productoFinal =
        newProducto != null
          ? QueryBuilder.validateId(newProducto)
          : oldProducto;
      ...
      if (productoFinal === oldProducto) {
        ...
        if (parseFloat(nuevoProducto.rows[0].stock) < cantidadFinal) {
          ...
          return responseHelper.error(
            res,
            `Stock insuficiente en el nuevo producto. Disponible: ${nuevoProducto.rows[0].stock}`,
            400
          );
        }
      ...
      const detalleActualizado = ModelMapper.toDetalleVenta(
        updateResult.rows[0]
      );
      ...
      logger.audit(
        ...
        "UPDATE",
        {
          detalle_id: id,
          cambios: {
            producto: productoFinal !== oldProducto,
  ...
  async getByVenta(req, res) {
    try {
      ...
      return responseHelper.error(
        res,
        "Error obteniendo detalles por venta",
        500,
        error
      );
    }
  ...
  async getByProducto(req, res) {
    try {
      ...
      return responseHelper.error(
        res,
        "Error obteniendo detalles por producto",
        500,
        error
      );
    }
  ...
  async createMultiple(req, res) {
    ...
    try {
      ...
      if (
        ...
        return responseHelper.error(
          res,
          "Datos inválidos para crear múltiples detalles",
          400
        );
      }
      ...
      for (let i = 0; i < detalles.length; i++) {
        ...
        try {
          ...
          if (precioNum <= 0) {
            errores.push(
              `Detalle ${i + 1}: El precio unitario debe ser mayor a 0`
            );
            continue;
          }
          ...
          if (parseFloat(producto.rows[0].stock) < cantidadNum) {
            errores.push(
              `Detalle ${i + 1}: Stock insuficiente. Disponible: ${
                producto.rows[0].stock
              }`
            );
            continue;
          }
          ...
          const detalleCreado = ModelMapper.toDetalleVenta(
            insertResult.rows[0]
          );
      ...
      if (errores.length > 0) {
        await transaction.query("ROLLBACK");
        return responseHelper.error(res, "Errores al crear detalles", 400, {
          errores,
        });
      }
      ...
      return responseHelper.error(
        res,
        "Error creando múltiples detalles de venta",
        500,
        error
      );
    } finally {
  ...
  async getReporteVentasProductos(req, res) {
    try {
      ...
      return responseHelper.error(
        res,
        "Error generando reporte de ventas por productos",
        500,
        error
      );
    }
  ...
  async getReporteTopProductos(req, res) {
    try {
      ...
      switch (periodo) {
        ...
        case "ytd":
          fechaFiltro =
            "EXTRACT(YEAR FROM v.fecha) = EXTRACT(YEAR FROM CURRENT_DATE)";
          break;
        default: // 30d
      ...
      return responseHelper.error(
        res,
        "Error generando reporte de top productos",
        500,
        error
      );
    }
  ...
  async getEstadisticasProducto(req, res) {
    try {
      ...
      // Reutilizar la lógica de estadísticas que ya tienes en getByProducto
      const statsResult = await db.query(
        `
      SELECT 
        COUNT(DISTINCT dv.id_venta) as total_ventas,
    ...
    `,
      ...
      return responseHelper.success(res, statsResult.rows[0]);
    } catch (error) {
      if (error.message === "ID inválido") {
        return responseHelper.error(res, "ID de producto inválido", 400);
      }
      logger.error("Error en getEstadisticasProducto:", error);
      return responseHelper.error(
        res,
        "Error obteniendo estadísticas del producto",
        500,
        error
      );
    }
  ...
  async getEstadisticasVenta(req, res) {
    ...
    `,
      ...
      logger.error("Error en getEstadisticasVenta:", error);
      return responseHelper.error(
        res,
        "Error obteniendo estadísticas de la venta",
        500,
        error
      );
    }
