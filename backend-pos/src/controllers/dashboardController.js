...
const dashboardController = {
        ...
        try {
            ...
            const productosPopulares = productosPopularesResult.rows.map(row => {
                ...
                const productoEnriquecido = {
                    ...
                    necesita_reposicion: producto.necesitaReposicion ? producto.necesitaReposicion() : false,
                    por_caducar: producto.estaPorCaducar ? producto.estaPorCaducar() : false
                };
            ...
            const metricasAdicionales = this.calcularMetricasAdicionales(estadisticasBasicas);
    ...
    async getMetricasRapidas(req, res) {
        ...
        try {
            ...
            const [
                ...
                `),
                client.query('SELECT COUNT(*) as total FROM alertas WHERE atendida = FALSE'),
                client.query('SELECT COUNT(*) as total FROM productos WHERE stock <= 5'),
                client.query(`
            ...
            return responseHelper.error(res, "Error obteniendo métricas rápidas", 500, error);
        } finally {
            client.release();
        }
    },
    ...
    async getEstadisticasAvanzadas(req, res) {
        ...
        try {
            ...
            const estadisticasAvanzadas = {
                ...
                metricas_eficiencia: {
                    tasa_conversion: parseFloat(metricasEficienciaResult.rows[0].tasa_conversion) || 0,
                    ticket_promedio: parseFloat(metricasEficienciaResult.rows[0].ticket_promedio),
                    frecuencia_compra: parseFloat(metricasEficienciaResult.rows[0].frecuencia_compra) || 0
                },
                ...
                resumen: {
                    ...
                    ventas_totales: ventasPeriodoResult.rows.reduce((sum, v) => sum + v.total_ventas, 0),
                    ingresos_totales: ventasPeriodoResult.rows.reduce((sum, v) => sum + parseFloat(v.ingresos), 0)
                }
            };
            ...
            return responseHelper.error(res, "Error generando estadísticas avanzadas", 500, error);
        } finally {
            client.release();
        }
    },
    ...
    async getAlertasDashboard(req, res) {
        ...
        try {
            ...
            const alertasClasificadas = {
                alta: alertas.filter(a => a.getPrioridad ? a.getPrioridad() === 'ALTA' : a.tipo === 'caducidad'),
                media: alertas.filter(a => a.getPrioridad ? a.getPrioridad() === 'MEDIA' : a.tipo === 'stock_bajo'),
            ...
            return responseHelper.error(res, "Error obteniendo alertas del dashboard", 500, error);
        } finally {
            client.release();
        }
    },
    ...
    calcularTendenciaVentas(ventasPorDia) {
        ...
        const porcentajeCambio = ((ultimosIngresos - primerosIngresos) / primerosIngresos) * 100;
    ...
    calcularMetricasAdicionales(estadisticasBasicas) {
        ...
        const proyeccionMensual = diasTranscurridos > 0 ? 
            (ingresosProyectados / diasTranscurridos) * diasMes : 0;

        return {
            proyeccion_ingresos_mensual: proyeccionMensual,
            eficiencia_diaria: estadisticasBasicas.ventas_hoy.ingresos > 0 ? 
                (estadisticasBasicas.ventas_hoy.ingresos / proyeccionMensual) * diasMes : 0,
            valor_usuario: estadisticasBasicas.total_usuarios > 0 ? 
                estadisticasBasicas.ventas_mes.ingresos / estadisticasBasicas.total_usuarios : 0
        };
    },
...
async getDatosVentas(req, res) {
        const client = await db.connect();
        try {
            const { tipo_grafico = 'lineas', periodo = '7d', agrupar_por = 'dia' } = req.query;
            ...
            const datosVentas = {
                ...
                resumen: {
                    total_ventas: ventasResult.rows.reduce((sum, v) => sum + parseInt(v.total_ventas), 0),
                    ingresos_totales: ventasResult.rows.reduce((sum, v) => sum + parseFloat(v.ingresos), 0),
                    ticket_promedio: ventasResult.rows.reduce((sum, v) => sum + parseFloat(v.promedio_venta), 0) / ventasResult.rows.length
                }
            };
            ...
            datosVentas.metodos_pago.forEach(metodo => {
                metodo.porcentaje = totalIngresos > 0 ? (metodo.ingresos / totalIngresos) * 100 : 0;
            });
            ...
            return responseHelper.error(res, "Error obteniendo datos de ventas", 500, error);
        } finally {
            client.release();
        }
    },
        ...
        try {
            ...
            const estadoInventario = {
                estadisticas: {
                    ...
                    productos_agotados: parseInt(inventarioResult.rows[0].productos_agotados),
                    productos_stock_bajo: parseInt(inventarioResult.rows[0].productos_stock_bajo),
                    productos_por_caducar: parseInt(inventarioResult.rows[0].productos_por_caducar)
                },
                ...
                alertas: {
                    necesita_atencion: parseInt(inventarioResult.rows[0].productos_agotados) + 
                                     parseInt(inventarioResult.rows[0].productos_stock_bajo) +
                                     parseInt(inventarioResult.rows[0].productos_por_caducar),
                    nivel_riesgo: this.calcularNivelRiesgoInventario(inventarioResult.rows[0])
                }
            };
            ...
            return responseHelper.error(res, "Error obteniendo estado de inventario", 500, error);
        } finally {
            client.release();
        }
    },
    ...
    async getMetricasFinancieras(req, res) {
        const client = await db.connect();
        try {
            const { moneda = 'MXN', incluir_proyecciones = true, nivel_detalle = 'basico' } = req.query;
            ...
            const metricasFinancieras = {
                ...
                ingresos_mensuales: ingresosMensualesResult.rows.map(row => ({
                    ...
                    gastos: this.obtenerGastosParaMes(gastosMensualesResult.rows, row.mes),
                    utilidad: parseFloat(row.ingresos) - this.obtenerGastosParaMes(gastosMensualesResult.rows, row.mes)
                })),
                ...
                resumen: {
                    ingresos_ultimo_mes: ingresosMensualesResult.rows[0] ? parseFloat(ingresosMensualesResult.rows[0].ingresos) : 0,
                    crecimiento_mensual: this.calcularCrecimientoMensual(ingresosMensualesResult.rows),
                    margen_promedio: margenGananciaResult.rows.reduce((sum, row) => sum + parseFloat(row.margen_porcentaje), 0) / margenGananciaResult.rows.length,
                    proyeccion_anual: this.calcularProyeccionAnual(ingresosMensualesResult.rows)
                }
            };
            ...
            return responseHelper.error(res, "Error obteniendo métricas financieras", 500, error);
        } finally {
            client.release();
        }
    },
        ...
        try {
            ...
            const estadisticasUsuarios = {
                ...
                actividad_reciente: actividadResult.rows.map(row => ({
                    ...
                    eficiencia: parseInt(row.total_ventas) > 0 ? parseFloat(row.ingresos_generados) / parseInt(row.total_ventas) : 0
                })),
                resumen: {
                    total_usuarios: usuariosResult.rows.reduce((sum, row) => sum + parseInt(row.total), 0),
                    usuarios_activos: usuariosResult.rows.reduce((sum, row) => sum + parseInt(row.activos), 0),
                    ventas_ultimo_mes: actividadResult.rows.reduce((sum, row) => sum + parseInt(row.total_ventas), 0),
                    ingresos_ultimo_mes: actividadResult.rows.reduce((sum, row) => sum + parseFloat(row.ingresos_generados), 0)
                }
            };
            ...
            return responseHelper.error(res, "Error obteniendo estadísticas de usuarios", 500, error);
        } finally {
            client.release();
        }
    },
    ...
    calcularNivelRiesgoInventario(estadisticas) {
        const productosProblema = estadisticas.productos_agotados + estadisticas.productos_stock_bajo + estadisticas.productos_por_caducar;
        const porcentajeProblema = (productosProblema / estadisticas.total_productos) * 100;
    ...
    obtenerGastosParaMes(gastos, mes) {
        const gastoMes = gastos.find(g => g.mes.getTime() === new Date(mes).getTime());
    ...
    calcularProyeccionAnual(ingresosMensuales) {
        ...
        const ingresoPromedio = ingresosMensuales.reduce((sum, row) => sum + parseFloat(row.ingresos), 0) / ingresosMensuales.length;
