...
const categoriaRepository = {
    ...
    async findAll({ searchTerm, limit, offset }) {
        ...
        dataSql += ` GROUP BY c.id_categoria ORDER BY c.nombre LIMIT $${params.length + 1} OFFSET $${params.length + 2}`;
        ...
        categorias.forEach(categoria => {
            const row = dataResult.rows.find(r => r.id_categoria === categoria.id_categoria);
    ...
    async findProductosByCategoriaId(id, { limit, offset }) {
        ...
        const [productosResult, totalResult] = await Promise.all([productsQuery, countQuery]);
    ...
    async getEstadisticasForCategoria(id) {
        ...
        const [statsResult, valorResult] = await Promise.all([statsQuery, valorQuery]);
